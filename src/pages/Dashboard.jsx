...
import { Database, Key, CreditCard, Activity, ArrowRight, Download, Settings } from 'lucide-react';
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
                    <div>
                        ...
                        <p className="text-secondary flex items-center gap-2">
                            ...
                            <span className="text-teal-400">{user.plan}</span>
                        </p>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="glass-panel p-8">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button className="text-sm text-teal-400 hover:text-teal-300 font-semibold">View All</button>
                            </div>
                                ...
                                {activeDatasets.map((ds, idx) => (
                                    <div key={idx} className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-purple-500/30 transition-colors">
                                        ...
                                        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-4">
                                            <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${ds.status === 'Active' ? 'bg-teal-500/10 text-teal-400 border border-teal-500/20' : 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20'}`}>
                                                {ds.status}
                                            </span>
                        ...
                        <div className="glass-panel p-8">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-primary flex items-center gap-2">
                                    <Key size={20} className="text-teal-400" /> API Keys
                                </h2>
                            ...
                            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-slate-700/50">
                                {recentActivity.map((log, idx) => (
                                    <div key={idx} className="relative flex items-start pl-8 md:pl-0">
                                        {/* Timeline Dot */}
                                        <div className="absolute left-0 md:left-1/2 w-4 h-4 rounded-full bg-slate-900 border-2 border-primary -translate-x-1.5 md:-translate-x-2 mt-1 z-10"></div>
